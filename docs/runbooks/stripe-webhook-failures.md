...
- В Compose **prod** монтируйте **`BILLING_DB_PATH`** на постоянный том (`docker-compose.prod.yml`), иначе после пересоздания контейнера история событий и идемпотентность теряются.
