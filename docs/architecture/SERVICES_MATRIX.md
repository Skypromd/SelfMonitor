...
- **security-proxy** (в `docker-compose.yml`): опциональный TLS-фронт к **security-service**; требует сертификаты/ключи в примонтированных путях — без них контейнер не поднимается. Для локальной разработки достаточно gateway **:8000** без proxy.
