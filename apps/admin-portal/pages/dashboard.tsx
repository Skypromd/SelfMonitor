...
export default function DashboardPage() {
  ...
  // ── Auth guard: read sessionStorage on mount ─────────────────────
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    const loadPartners = async () => {
      try {
        ...
        if (!response.ok) throw new Error('Failed to load partners for billing filters.');
  ...
  const selectedStatuses = useMemo(() => {
    ...
    if (includeQualified) statuses.push('qualified');
    if (includeConverted) statuses.push('converted');
    return statuses;
  ...
  const maxPartnerAmount = useMemo(() => {
    if (!billingRowsSorted.length) return 1;
    return Math.max(...billingRowsSorted.map((item) => item.amount_gbp), 1);
  }, [billingRowsSorted]);

  const chartPoints = useMemo(() => {
    if (!billingRowsSorted.length) return '';
    if (billingRowsSorted.length === 1) return '0,75 100,75';
  ...
  useEffect(() => {
    if (!startDate && !endDate && activePreset === '30d') applyPreset('30d');
  }, [activePreset, endDate, startDate]);

  useEffect(() => {
    if (!selectedPartnerModal) return;
    const onEscape = (event: KeyboardEvent) => {
      if (event.key === 'Escape') setSelectedPartnerModal(null);
    };
  ...
  const loadLeadOps = async () => {
    ...
    try {
      const params = new URLSearchParams();
      if (selectedPartnerId) params.set('partner_id', selectedPartnerId);
      if (leadOpsStatusFilter !== 'all') params.set('status', leadOpsStatusFilter);
      if (leadOpsUserFilter) params.set('user_id', leadOpsUserFilter);
      params.set('limit', '25');
      params.set('offset', '0');
      ...
      if (!response.ok) throw new Error((payload as { detail?: string }).detail || 'Failed to load leads');
  ...
  const refreshInvoiceList = async () => {
    ...
    try {
      ...
      if (!response.ok) throw new Error((payload as { detail?: string }).detail || 'Failed to load invoices');
      const data = payload as InvoiceListResponse;
      setInvoiceRows(data.items);
      setInvoiceStatusDrafts(
        data.items.reduce<Record<string, InvoiceSummary['status']>>((acc, item) => {
  ...
  const generateInvoiceSnapshot = async () => {
    if (!selectedStatuses.length) { pushToast('error', 'Select at least one billable status.'); return; }
    setIsInvoiceGenerateLoading(true);
    try {
      const body: { end_date?: string; partner_id?: string; start_date?: string; statuses: string[] } = { statuses: selectedStatuses };
      if (selectedPartnerId) body.partner_id = selectedPartnerId;
      ...
      if (!response.ok) throw new Error(payload.detail || 'Failed to generate invoice');
  ...
  const updateInvoiceStatus = async (invoiceId: string) => {
    ...
    try {
      ...
      const payload = await response.json();
      if (!response.ok) throw new Error(payload.detail || 'Failed to update invoice status');
  ...
  const downloadInvoiceArtifact = async (invoice: InvoiceSummary, target: 'pdf' | 'xero' | 'quickbooks') => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        try { const p = await response.json(); if (p?.detail) detail = p.detail as string; } catch { /* keep fallback */ }
        throw new Error(detail);
      }
      const blob = await response.blob();
      const disposition = response.headers.get('content-disposition');
      const matched = disposition?.match(/filename="?([^"]+)"?/i);
      const fallbackName = target === 'pdf' ? `${invoice.invoice_number}.pdf` : `${invoice.invoice_number}_${target}.csv`;
  ...
  const handlePricingUpdate = async (event: FormEvent) => {
    ...
    if (!pricingPartnerId) { pushToast('error', 'Select a partner first.'); return; }
    ...
    try {
      const response = await fetch(`${PARTNER_REGISTRY_URL}/partners/${pricingPartnerId}/pricing`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` },
        body: JSON.stringify({ qualified_lead_fee_gbp: qualifiedValue, converted_lead_fee_gbp: convertedValue }),
      });
      const payload = await response.json();
      if (!response.ok) throw new Error(payload.detail || 'Failed to update partner pricing');
      setPartners((current) =>
        current.map((partner) =>
          partner.id === pricingPartnerId
            ? { ...partner, qualified_lead_fee_gbp: payload.qualified_lead_fee_gbp, converted_lead_fee_gbp: payload.converted_lead_fee_gbp }
            : partner,
        ),
      );
  ...
  const loadBillingReport = async () => {
    ...
    try {
      ...
      const payload = await response.json();
      if (!response.ok) throw new Error(payload.detail || 'Failed to load billing report');
      setReport(payload);
  ...
  const handleLeadStatusUpdate = async (event: FormEvent) => {
    event.preventDefault();
    if (!leadId) { pushToast('error', 'Lead ID is required.'); return; }
    ...
    try {
      ...
      const payload = await response.json();
      if (!response.ok) throw new Error(payload.detail || 'Failed to update lead status');
  ...
  useEffect(() => {
    if (token) void refreshInvoiceList();
  }, [token]);
  ...
  const statusColor = (s: string) => s === 'online' ? '#34d399' : s === 'checking' ? '#f59e0b' : '#f87171';
  ...
  return (
    <div className={styles.dashboard}>
              ...
              {MOCK_PLAN_STATS.map((p) => (
                <div key={p.plan} className={styles.planDistCard} style={{ borderColor: p.color }}>
                  ...
                  <div className={styles.planDistBar}>
                    <div className={styles.planDistBarFill} style={{ width: `${(p.count / PLAN_MAX) * 100}%`, background: p.color }} />
                  </div>
          ...
          <div className={styles.subContainer}>
            ...
            <div className={styles.tableResponsive}>
              <table className={styles.table}>
                <thead><tr><th>Email</th><th>Plan</th><th>Status</th><th>Joined</th><th>Revenue (£)</th></tr></thead>
                <tbody>
                  {MOCK_USERS.slice(0, 5).map((u) => (
                    <tr key={u.id}>
                      <td>{u.email}</td>
                      <td><span className={styles.planBadge}>{u.plan}</span></td>
                      <td><span style={{ color: u.status === 'active' ? '#34d399' : u.status === 'trialing' ? '#f59e0b' : '#f87171', fontWeight: 600, textTransform: 'capitalize' }}>{u.status}</span></td>
                      <td>{u.joined}</td>
      ...
      {activeTab === 'subscriptions' && (() => {
        const planColors: Record<string, string> = { starter: '#14b8a6', growth: '#6366f1', pro: '#f59e0b', business: '#ec4899', free: '#64748b' };
        const planPrices: Record<string, number> = { free: 0, starter: 9, growth: 12, pro: 15, business: 25 };
        const activePlans = billingAdminStats?.by_plan ?? MOCK_PLAN_STATS.map((p) => ({ plan: p.plan.toLowerCase(), count: p.count, mrr: p.mrr, active: p.count, trialing: 0, inactive: 0 }));
        ...
        return (
          <>
            <div style={{ display: 'inline-flex', alignItems: 'center', gap: '0.5rem', padding: '0.3rem 0.75rem', borderRadius: 20, background: isLive ? 'rgba(52,211,153,0.1)' : 'rgba(251,191,36,0.1)', border: `1px solid ${isLive ? 'rgba(52,211,153,0.3)' : 'rgba(251,191,36,0.3)'}`, color: isLive ? '#34d399' : '#fbbf24', fontSize: '0.75rem', marginBottom: '1.25rem' }}>
              {isBillingStatsLoading ? '⏳ Loading live data…' : isLive ? '🟢 Live data — Stripe billing-service' : '🟡 Mock data — billing-service unavailable'}
            </div>
                ...
                return (
                  <div key={p.plan} className={styles.kpiCard} style={{ borderLeft: `3px solid ${color}` }}>
                    ...
                    <p className={styles.kpiSub}>
                      ...
                      {isLive ? ` · ${p.active} active, ${p.trialing} trial` : ` · £${price}/user`}
                    </p>
                  </div>
                );
              })}
            ...
            <div className={styles.subContainer}>
              ...
              <div className={styles.tableResponsive}>
                <table className={styles.table}>
                  <thead>
                    <tr>
                      <th>Plan</th><th>Price/mo</th><th>Subscribers</th>
                      {isLive && <th>Active</th>}
                      {isLive && <th>Trialing</th>}
                      <th>MRR (£)</th><th>ARR (£)</th><th>% of MRR</th><th>Revenue Bar</th>
                    </tr>
                  </thead>
                  <tbody>
                    {activePlans.map((p) => {
            ...
            {isLive && recentSubs.length > 0 && (
              <div className={styles.subContainer}>
                ...
                <div className={styles.tableResponsive}>
                  <table className={styles.table}>
                    <thead><tr><th>Email</th><th>Plan</th><th>Status</th><th>Stripe</th><th>Date</th></tr></thead>
                    <tbody>
                      {recentSubs.map((s, i) => (
                        <tr key={i}>
                          <td>{s.email}</td>
        ...
        <>
          <div className={styles.subContainer}>
            ...
            <div className={styles.adminFiltersGrid}>
              <label className={styles.filterField}>
                ...
                <input className={styles.input} onChange={(e) => setUserSearch(e.target.value)} placeholder="user@example.com" type="text" value={userSearch} />
              </label>
              <label className={styles.filterField}>
                <span>Filter by plan</span>
                <select className={styles.categorySelect} onChange={(e) => setUserPlanFilter(e.target.value)} value={userPlanFilter}>
            ...
            <div className={styles.tableResponsive}>
              <table className={styles.table}>
                <thead><tr><th>Email</th><th>Plan</th><th>Status</th><th>Joined</th><th>Next Bill</th><th>Revenue (£)</th><th>Actions</th></tr></thead>
                <tbody>
                  {filteredUsers.length === 0 && (
                    <tr><td colSpan={7}><p className={styles.emptyState}>No users match the current filter.</p></td></tr>
                  )}
                  {filteredUsers.map((u) => (
                    <tr key={u.id}>
                      <td>{u.email}</td>
                      ...
                      <td>{u.nextBill}</td>
                      <td className={u.revenue > 0 ? styles.positive : ''}>{u.revenue > 0 ? `£${u.revenue}` : '—'}</td>
                      <td><button className={styles.tableActionButton} onClick={() => pushToast('info', `User ${u.email} details — connect live API to action.`)} type="button">Manage</button></td>
                    </tr>
                  ))}
                </tbody>
              </table>
          ...
          <div className={styles.subContainer}>
            ...
            <form onSubmit={handleDeactivate}>
              <input className={styles.input} onChange={(event) => setEmailToDeactivate(event.target.value)} placeholder="user.email@example.com" type="email" value={emailToDeactivate} />
        ...
        <>
          <div className={styles.subContainer}>
            ...
            <form className={styles.adminStatusForm} onSubmit={handlePricingUpdate}>
              <select className={styles.categorySelect} onChange={(event) => setPricingPartnerId(event.target.value)} value={pricingPartnerId}>
                <option value="" disabled>Select partner</option>
                {partners.map((partner) => (<option key={partner.id} value={partner.id}>{partner.name}</option>))}
              </select>
              <input className={styles.input} onChange={(event) => setQualifiedPricing(event.target.value)} placeholder="Qualified fee GBP" type="number" min="0" step="0.01" value={qualifiedPricing} />
              <input className={styles.input} onChange={(event) => setConvertedPricing(event.target.value)} placeholder="Converted fee GBP" type="number" min="0" step="0.01" value={convertedPricing} />
              <button className={styles.button} disabled={isPricingLoading} type="submit">{isPricingLoading ? 'Saving...' : 'Save Pricing'}</button>
            </form>
          </div>
        </>
      )}
        ...
        <>
          <div className={styles.subContainer}>
            ...
            <form className={styles.adminStatusForm} onSubmit={handleLeadStatusUpdate}>
              <input className={styles.input} onChange={(event) => setLeadId(event.target.value)} placeholder="Lead UUID" type="text" value={leadId} />
              <select className={styles.categorySelect} onChange={(event) => setLeadStatus(event.target.value as LeadLifecycleStatus)} value={leadStatus}>
                <option value="qualified">qualified</option>
                <option value="rejected">rejected</option>
                <option value="converted">converted</option>
              </select>
              <button className={styles.button} disabled={isLeadActionLoading} type="submit">{isLeadActionLoading ? 'Updating...' : 'Update Lead Status'}</button>
            </form>
          </div>
          ...
          <div className={styles.subContainer}>
            ...
            <div className={styles.adminFiltersGrid}>
              <label className={styles.filterField}>
                <span>Status</span>
                <select className={styles.categorySelect} onChange={(event) => setLeadOpsStatusFilter(event.target.value as 'all' | 'initiated' | 'qualified' | 'rejected' | 'converted')} value={leadOpsStatusFilter}>
              ...
              <label className={styles.filterField}>
                <span>User ID</span>
                <input className={styles.input} onChange={(event) => setLeadOpsUserFilter(event.target.value)} placeholder="user@example.com" type="text" value={leadOpsUserFilter} />
              </label>
              <div className={styles.filterField}>
                <span>Actions</span>
                <button className={styles.button} disabled={isLeadOpsLoading} onClick={loadLeadOps} type="button">{isLeadOpsLoading ? 'Loading...' : 'Load Leads'}</button>
              </div>
            </div>
            <p className={styles.tableCaption}>Total matching leads: {leadOpsTotal}</p>
            <div className={styles.tableResponsive}>
              <table className={styles.table}>
                <thead><tr><th>Created</th><th>User</th><th>Partner</th><th>Status</th><th>Lead ID</th></tr></thead>
                <tbody>
                  {leadOpsRows.length === 0 && (<tr><td colSpan={5}><p className={styles.emptyState}>No leads loaded yet.</p></td></tr>)}
                  {leadOpsRows.map((row) => (
                    <tr key={row.id}>
          ...
          <div className={styles.subContainer}>
            ...
            <div className={styles.billingControlPanel}>
              <div className={styles.presetRow}>
                {(['7d', '30d', 'qtd', 'custom'] as PeriodPreset[]).map((preset) => (
                  <button key={preset} className={`${styles.presetButton} ${activePreset === preset ? styles.presetButtonActive : ''}`} onClick={() => applyPreset(preset)} type="button">
                    {preset.toUpperCase()}
                  </button>
                ))}
              </div>
              ...
              <div className={styles.adminFiltersGrid}>
                <label className={styles.filterField}>
                  ...
                  <input className={styles.input} onChange={(event) => { setStartDate(event.target.value); setActivePreset('custom'); }} type="date" value={startDate} />
                </label>
                <label className={styles.filterField}>
                  <span>End date</span>
                  <input className={styles.input} onChange={(event) => { setEndDate(event.target.value); setActivePreset('custom'); }} type="date" value={endDate} />
                </label>
                <label className={styles.filterField}>
                  <span>Partner</span>
              ...
              <div className={styles.adminActionsRow}>
                <button className={styles.button} disabled={isBillingLoading} onClick={loadBillingReport} type="button">{isBillingLoading ? 'Loading...' : 'Load Billing'}</button>
                <button className={`${styles.button} ${styles.secondaryButton}`} onClick={downloadBillingCsv} type="button">Download CSV</button>
              </div>
            </div>
            ...
            {report && (
              <>
                <div className={styles.kpiGrid}>
                  <div className={styles.kpiCard}><p className={styles.kpiLabel}>Total revenue</p><p className={styles.kpiValue}>{currency(report.total_amount_gbp, report.currency)}</p></div>
                  <div className={styles.kpiCard}><p className={styles.kpiLabel}>Qualified leads</p><p className={styles.kpiValue}>{report.qualified_leads}</p></div>
                  <div className={styles.kpiCard}><p className={styles.kpiLabel}>Converted leads</p><p className={styles.kpiValue}>{report.converted_leads}</p></div>
                  <div className={styles.kpiCard}><p className={styles.kpiLabel}>Unique users</p><p className={styles.kpiValue}>{report.unique_users}</p></div>
                </div>
                ...
                <div className={styles.tableResponsive}>
                  <table className={styles.table}>
                    <thead><tr><th>Partner</th><th>Qualified</th><th>Converted</th><th>Rates (GBP)</th><th>Amount (GBP)</th><th>Action</th></tr></thead>
                    <tbody>
                      {billingRowsSorted.map((item) => (
                        <tr key={item.partner_id}>
                          ...
                          <td><button className={styles.tableActionButton} onClick={() => setSelectedPartnerModal(item)} type="button">Details</button></td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
      ...
      {activeTab === 'invoices' && (
        <>
          <div className={styles.subContainer}>
            ...
            <div className={styles.adminActionsRow}>
              <button className={styles.button} disabled={isInvoiceGenerateLoading} onClick={generateInvoiceSnapshot} type="button">{isInvoiceGenerateLoading ? 'Generating...' : 'Generate Invoice Snapshot'}</button>
              <button className={`${styles.button} ${styles.secondaryButton}`} disabled={isInvoiceListLoading} onClick={refreshInvoiceList} type="button">{isInvoiceListLoading ? 'Refreshing...' : 'Refresh Invoice Queue'}</button>
            </div>
            <div className={styles.tableResponsive}>
              <table className={styles.table}>
                <thead><tr><th>Created</th><th>Invoice #</th><th>Due date</th><th>Status</th><th>Total</th><th>Exports</th><th>Update</th></tr></thead>
                <tbody>
                  {invoiceRows.length === 0 && (<tr><td colSpan={7}><p className={styles.emptyState}>No invoices generated yet.</p></td></tr>)}
                    ...
                    <tr key={invoice.id}>
                      ...
                      <td>
                        <div className={styles.invoiceExportGroup}>
                          {(['pdf', 'xero', 'quickbooks'] as const).map((target) => (
                            <button key={target} className={styles.tableActionButton} disabled={invoiceExportLoadingKey === `${invoice.id}:${target}`} onClick={() => downloadInvoiceArtifact(invoice, target)} type="button">
                              {invoiceExportLoadingKey === `${invoice.id}:${target}` ? '...' : target === 'pdf' ? 'PDF' : target.charAt(0).toUpperCase() + target.slice(1)}
                            </button>
                          ))}
                        </div>
                      </td>
                      <td>
                        <div className={styles.invoiceActionGroup}>
                          <select className={styles.categorySelect} onChange={(event) => setInvoiceStatusDrafts((current) => ({ ...current, [invoice.id]: event.target.value as InvoiceSummary['status'] }))} value={invoiceStatusDrafts[invoice.id] || invoice.status}>
                            <option value="generated">generated</option>
                            <option value="issued">issued</option>
                            <option value="paid">paid</option>
                            <option value="void">void</option>
                          </select>
                          <button className={styles.tableActionButton} disabled={isInvoiceStatusLoading === invoice.id} onClick={() => updateInvoiceStatus(invoice.id)} type="button">
        ...
        <>
          <div className={styles.subContainer}>
            <div className={styles.sectionHeader}>
              ...
              <p className={styles.sectionSubtitle}>Delegate repetitive admin tasks to your AI agent. Enable permissions one by one as you gain confidence. The agent will log every action it takes — you stay in full control.</p>
            </div>
            <div className={styles.agentStatusRow}>
              <div className={styles.agentStatusLabel}>
                <span style={{ fontSize: '1.5rem' }}>{agentEnabled ? '🟢' : '🔴'}</span>
                <div>
                  <strong style={{ color: '#f1f5f9', fontSize: '1.1rem' }}>AI Agent {agentEnabled ? 'ENABLED' : 'DISABLED'}</strong>
                  <p style={{ color: '#94a3b8', fontSize: '0.85rem', margin: 0 }}>
                    {agentEnabled ? 'Agent is active and monitoring. It will act only within permitted scopes.' : 'Agent is paused. No automated actions will be taken.'}
                  </p>
                </div>
              </div>
              <button className={agentEnabled ? styles.button : `${styles.button} ${styles.secondaryButton}`} onClick={() => setAgentEnabled((v) => !v)} style={{ background: agentEnabled ? '#14b8a6' : undefined }} type="button">
              ...
              {AI_AGENT_PERMISSIONS.map((perm) => (
                <div key={perm.id} className={styles.permRow}>
                  <label className={styles.permToggle}>
                    <input type="checkbox" checked={perm.available ? agentPerms[perm.id] : false} disabled={!perm.available} onChange={(e) => setAgentPerms((prev) => ({ ...prev, [perm.id]: e.target.checked }))} />
                    <span className={styles.permLabel}>{perm.available ? '✅' : '🚧'} {perm.label}</span>
                  </label>
                  {!perm.available && <span className={styles.permComingSoon}>Coming Soon</span>}
                </div>
              ))}
            </div>
          </div>
              ...
              <table className={styles.table}>
                ...
                <tbody>
                  {[
                    ...
                    <tr key={row.task}>
                      <td>{row.task}</td><td>{row.schedule}</td><td>{row.last}</td><td>{row.next}</td>
                      <td><span style={{ color: row.status === 'active' ? '#34d399' : '#f59e0b', textTransform: 'capitalize', fontWeight: 600 }}>{row.status}</span></td>
                    </tr>
                  ))}
                </tbody>
              </table>
          ...
          <div className={styles.subContainer}>
            ...
            <div className={styles.tableResponsive}>
              <table className={styles.table}>
                ...
                <tbody>
                  <tr><td colSpan={4}><p className={styles.emptyState}>{agentEnabled ? 'Agent active — no actions taken yet in this session.' : 'Agent is disabled. Enable agent to start recording actions.'}</p></td></tr>
                </tbody>
              </table>
            </div>
          </div>
        ...
        <>
          <div className={styles.subContainer}>
            <div className={styles.sectionHeader}>
              ...
              <p className={styles.sectionSubtitle}>Live health checks for all {SYSTEM_SERVICES.length} microservices. Click the button to ping all endpoints.</p>
            </div>
            <div className={styles.adminActionsRow}>
              <button className={styles.button} onClick={checkAllServices} type="button">🔍 Ping All Services</button>
            </div>
          ...
          <div className={styles.subContainer}>
            ...
            <div className={styles.tableResponsive}>
              <table className={styles.table}>
                <thead><tr><th>Service</th><th>Uptime 30d</th><th>Avg Response</th><th>Incidents</th><th>SLA Target</th></tr></thead>
                <tbody>
                  {[
        ...
        <>
          <div className={styles.kpiGrid}>
            ...
            <div className={styles.kpiCard}>
              ...
              <p className={styles.kpiValue} style={{ color: '#f87171' }}>{MOCK_TICKETS.filter((t) => t.status === 'open').length}</p>
            ...
            <div className={styles.kpiCard}>
              ...
              <p className={styles.kpiValue} style={{ color: '#f59e0b' }}>{(MOCK_FEEDBACK.reduce((s, f) => s + f.rating, 0) / MOCK_FEEDBACK.length).toFixed(1)} ⭐</p>
          ...
          <div className={styles.subContainer}>
            ...
            <table className={styles.dataTable}>
              <thead>
                <tr><th>ID</th><th>User</th><th>Subject</th><th>Category</th><th>Priority</th><th>Status</th><th>Created</th></tr>
              </thead>
              <tbody>
                {MOCK_TICKETS.map((ticket) => (
                  <tr key={ticket.id}>
                    <td><code style={{ fontSize: '.8rem' }}>{ticket.id}</code></td>
                    <td style={{ fontSize: '.85rem' }}>{ticket.user}</td>
                    <td style={{ maxWidth: 220, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{ticket.subject}</td>
                    <td>{ticket.category}</td>
                    <td>
                      <span style={{ padding: '.2rem .55rem', borderRadius: 20, fontSize: '.78rem', fontWeight: 600, background: ticket.priority === 'high' ? '#7f1d1d' : ticket.priority === 'medium' ? '#78350f' : '#14532d', color: ticket.priority === 'high' ? '#f87171' : ticket.priority === 'medium' ? '#fbbf24' : '#34d399' }}>
                        {ticket.priority}
                      </span>
                    </td>
                    <td>
                      <span style={{ padding: '.2rem .55rem', borderRadius: 20, fontSize: '.78rem', fontWeight: 600, background: ticket.status === 'open' ? '#1e3a5f' : '#14532d', color: ticket.status === 'open' ? '#93c5fd' : '#34d399' }}>
                        {ticket.status}
                      </span>
                    </td>
                    <td style={{ fontSize: '.82rem', color: 'var(--lp-text-muted)' }}>{ticket.created}</td>
                  </tr>
                ))}
              </tbody>
            </table>
          ...
          <div className={styles.subContainer} style={{ marginTop: '1.5rem' }}>
            ...
            <table className={styles.dataTable}>
              <thead><tr><th>Date</th><th>User</th><th>Rating</th><th>Comment</th></tr></thead>
              <tbody>
                {MOCK_FEEDBACK.map((f, i) => (
                  <tr key={i}>
                    <td style={{ fontSize: '.82rem', color: 'var(--lp-text-muted)' }}>{f.date}</td>
        ...
        <div className={styles.modalBackdrop} onClick={() => setSelectedPartnerModal(null)} role="presentation">
          <div className={styles.modalCard} onClick={(event) => event.stopPropagation()} role="dialog" aria-modal="true">
            <div className={styles.modalHeader}>
              ...
              <button aria-label="Close partner details" className={styles.toastClose} onClick={() => setSelectedPartnerModal(null)} type="button">×</button>
            </div>
            <p className={styles.sectionSubtitle}>Partner billing drill-down for selected filters.</p>
            <div className={styles.modalGrid}>
              <div className={styles.modalMetric}><span className={styles.modalLabel}>Qualified leads</span><strong className={styles.modalValue}>{selectedPartnerModal.qualified_leads}</strong></div>
              <div className={styles.modalMetric}><span className={styles.modalLabel}>Converted leads</span><strong className={styles.modalValue}>{selectedPartnerModal.converted_leads}</strong></div>
              <div className={styles.modalMetric}><span className={styles.modalLabel}>Unique users</span><strong className={styles.modalValue}>{selectedPartnerModal.unique_users}</strong></div>
              <div className={styles.modalMetric}><span className={styles.modalLabel}>Revenue contribution</span><strong className={styles.modalValue}>{currency(selectedPartnerModal.amount_gbp, report.currency)}</strong></div>
              <div className={styles.modalMetric}><span className={styles.modalLabel}>Conversion rate</span><strong className={styles.modalValue}>{selectedPartnerModal.qualified_leads ? `${((selectedPartnerModal.converted_leads / selectedPartnerModal.qualified_leads) * 100).toFixed(1)}%` : '0.0%'}</strong></div>
              <div className={styles.modalMetric}><span className={styles.modalLabel}>Avg revenue / user</span><strong className={styles.modalValue}>{selectedPartnerModal.unique_users ? currency(selectedPartnerModal.amount_gbp / selectedPartnerModal.unique_users, report.currency) : currency(0, report.currency)}</strong></div>
            </div>
          </div>
        </div>
      )}
      ...
      <div className={styles.toastViewport}>
        {toasts.map((toast) => (
          <div className={`${styles.toastItem} ${toast.kind === 'success' ? styles.toastSuccess : toast.kind === 'error' ? styles.toastError : styles.toastInfo}`} key={toast.id} role="status">
            <span>{toast.message}</span>
            <button aria-label="Dismiss notification" className={styles.toastClose} onClick={() => dismissToast(toast.id)} type="button">×</button>
          </div>
        ))}
      </div>
    </div>
